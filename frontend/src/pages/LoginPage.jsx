...
import { ArrowLeft, Loader2, Lock, LogIn, Mail, Rocket } from 'lucide-react'
...
export default function LoginPage() {
  ...
  const inputIconStyle = { backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.1)', color: '#fff', transition: 'all 0.3s ease', width: '100%', borderRadius: '0.75rem', padding: '0.75rem 1rem 0.75rem 2.5rem', fontSize: '0.875rem', fontFamily: F, outline: 'none' }
  ...
  return (
    <>
      <style>{`
        ...
        .lp-input:focus { border-color: #FF0055 !important; box-shadow: 0 0 10px rgba(255,0,85,0.2) !important; }
        .lp-input::placeholder { color: #6b7280; }
        .lp-btn { background:linear-gradient(90deg,#FF0055,#d40046); box-shadow:0 0 20px rgba(255,0,85,0.4); transition:all 0.3s ease; border:none; cursor:pointer; color:#fff; font-family:'Plus Jakarta Sans',sans-serif; }
        .lp-btn:hover:not(:disabled) { box-shadow:0 0 35px rgba(255,0,85,0.7); transform:translateY(-2px); }
        .lp-btn:disabled { opacity:0.6; cursor:not-allowed; transform:none; }
        .lp-link { color:#f87171; font-weight:700; text-decoration:none; transition:color 0.2s; }
        .lp-link:hover { color:#fca5a5; }
        .lp-back { color:#9ca3af; text-decoration:none; font-size:0.875rem; font-weight:500; display:flex; align-items:center; gap:0.5rem; transition:color 0.2s; font-family:'Plus Jakarta Sans',sans-serif; }
        .lp-back:hover { color:#fff; }
        .lp-tg { background:linear-gradient(90deg,#FF0055,#FF5500); -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent; }
        @keyframes spin { to { transform: rotate(360deg); } }
        .lp-spin { animation: spin 1s linear infinite; }
        @media(max-width:1023px) { .lp-left { display:none!important; } .lp-right { width:100%!important; } }
      `}</style>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#08090c', color: '#fff', fontFamily: F, overflowX: 'hidden' }}>
        ...
        <nav style={{ ...GL, position: 'fixed', top: 0, left: 0, width: '100%', zIndex: 50, padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <Link to="/" className="lp-back"><ArrowLeft style={{ width: '1rem', height: '1rem' }} /> Back to Home</Link>
        </nav>
        ...
        <div style={{ display: 'flex', flex: 1, paddingTop: '72px' }}>
          ...
          {/* LEFT PANEL */}
          <div className="lp-left" style={{ display: 'flex', width: '50%', position: 'relative', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', borderRight: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <div style={{ position: 'relative', zIndex: 20, textAlign: 'center', padding: '0 3rem' }}>
              ...
              <h2 style={{ fontSize: '3rem', fontWeight: 900, lineHeight: 1.25, marginBottom: '1.5rem', color: '#fff', fontFamily: F }}>
                Welcome to the <br /><span className="lp-tg">Future Skills Guild</span>
              </h2>
          ...
          {/* RIGHT PANEL */}
          <div className="lp-right" style={{ width: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', backgroundColor: '#0c0d12', position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ ...GL, width: '100%', maxWidth: '28rem', padding: '2.5rem', borderRadius: '1.5rem', position: 'relative', zIndex: 10 }}>
              <div style={{ textAlign: 'center', marginBottom: '2.5rem' }}>
                ...
                <h3 style={{ fontSize: '1.875rem', fontWeight: 900, marginBottom: '0.5rem', color: '#fff', fontFamily: F }}>Welcome Back</h3>
                <p style={{ fontSize: '0.875rem', color: '#9ca3af', fontFamily: F }}>Sign in to your CareerBridge AI account</p>
              </div>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <Mail style={iconPos} />
                    <input type="email" id="email" autoComplete="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder="name@example.com" className="lp-input" style={inputIconStyle} />
                  </div>
                </div>
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <Lock style={iconPos} />
                    <input type="password" id="password" autoComplete="current-password" required value={password} onChange={e => setPassword(e.target.value)} placeholder="••••••••" className="lp-input" style={inputIconStyle} />
                  </div>
                </div>

                {error && <p style={{ textAlign: 'center', fontSize: '0.875rem', color: '#f87171', fontFamily: F }} role="alert">{error}</p>}

                <button type="submit" disabled={loading} className="lp-btn" style={{ width: '100%', padding: '1rem', borderRadius: '0.75rem', fontSize: '1.125rem', fontWeight: 700, letterSpacing: '0.025em', marginTop: '0.5rem', display: 'flex', justifyContent: 'center', alignItems: 'center', gap: '0.5rem' }}>
                  {loading ? (<><Loader2 className="lp-spin" style={{ width: '1.25rem', height: '1.25rem' }} /> Signing in…</>) : (<>Login <LogIn style={{ width: '1.25rem', height: '1.25rem' }} /></>)}
                </button>
              </form>
